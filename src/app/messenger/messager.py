...
class Messager(LogWrapper):
    ...
    def process_queue(self):
        ...
        # FIXME: it should be the current queue task
        ...
        for _new in all_news:
            ...
            if "urlPhoto" in _new:
                r = telegram.send_photo(_id, _new["urlPhoto"], message_header)
                if r is False:
                    # FIXME: replace with Queue
                    self.db.set_as_error_in_messager_queue(_new["url"])
                    continue
            r = telegram.send_message(_id, message)
            if r is False:
                # FIXME: replace with Queue
                self.db.set_as_error_in_messager_queue(_new["url"])
                continue
