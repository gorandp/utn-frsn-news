...
wordpress_regex_url = re.compile(r"https://www.frsn.utn.edu.ar/\?p=(\d+)")
...
class BotDb(Base):
    ...
    def insert_messager_queue(self, news: list) -> list:
        ...
        self.logger.info(f"{len(messager_queue)} elements inserted in messager queue")

    def get_unprocessed_messager_queue(self) -> list:
        result = self.db.database["messagerQueue"].find({"status": 0})
    ...
    def get_news(self, messagerQueue: list) -> list:
        ...
        result = self.db.database["news"].find({"url": {"$in": urlQueue}})
        result = sorted([r for r in result], key=lambda doc: get_order(doc["url"]))
        return result
