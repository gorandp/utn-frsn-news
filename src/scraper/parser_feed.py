...
class HistoricFeed(LogWrapper):
    ...
    def get_urls(self, limit: int | None = None):
    ...
    # def get_urls_old_site(self, limit: int | None = None):
    #     response = requests.get(OLD_SITE.HISTORIC_FEED_URL, timeout=OLD_SITE.TIMEOUT)
    ...
    #         if photo_url:
    #             photo_url = "https://wwwsi.frsn.utn.edu.ar" + photo_url.attrs["src"]

    #         urls.append((new_url, photo_url))
    #     return urls


# class ShortFeed(LogWrapper):
#     def __init__(self):
#         super().__init__(__name__)

#     def get_urls(self):
#         response = requests.get(OLD_SITE.SHORT_FEED_URL, timeout=OLD_SITE.TIMEOUT)
#         parser = bs4.BeautifulSoup(
#             response.content.decode("utf-8", errors="ignore"), "lxml"
#         )
#         urls = []
